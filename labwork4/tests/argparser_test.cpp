...
TEST(ArgParserTestSuite, MultiValueStringTest1) {
    ArgParser parser("My Parser");
    parser.AddStringArgument('a', "param1").MultiValue(2);

    ASSERT_TRUE(parser.Parse(SplitString("app --param1=value1 --param1=value2")));
    ASSERT_EQ(parser.GetStringValue("param1", 1), "value2");
}

TEST(ArgParserTestSuite, MultiValueStringTest2) {
    ArgParser parser("My Parser");
    parser.AddStringArgument('a', "param1").MultiValue(2);

    ASSERT_FALSE(parser.Parse(SplitString("app --param1=value1")));
}
...
TEST(ArgParserTestSuite, IntTestAndIntShortName) {
    ArgParser parser("My Parser");
    parser.AddIntArgument('a', "param1", "Some int argument");
    parser.AddIntArgument('b', "param2", "Some int argument");

    ASSERT_TRUE(parser.Parse(SplitString("app --param1 100500 -b 599")));
    ASSERT_EQ(parser.GetIntValue("param1"), 100500);
    ASSERT_EQ(parser.GetIntValue("param2"), 599);
}
...
TEST(ArgParserTestSuite, HelpStringTest) {
    ...
    parser.AddIntArgument("number", "Some Number").MultiValue().Positional();
    ...
    ASSERT_EQ(
        ...
        "     --number=<int>,  Some Number [repeated, min args = 0, positional]\n"
        "\n"
